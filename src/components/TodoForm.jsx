  ...
  return (
    <Modal open={open} onClose={onClose}>
      <Box sx={style}>
        ...
        <DialogContent>
          <TextField
            fullWidth label="TODO Text *" value={text} onChange={e => setText(e.target.value)}
            sx={{ mb: 2 }} required
          />
          <DateTimePicker
          ...
          />
          <TextField type="file" fullWidth accept="image/*" onChange={e => setImage(e.target.files[0] || null)}
            InputProps={{ startAdornment: <InputAdornment position="start"><AddAPhoto /></InputAdornment> }}
            sx={{ mb: 2 }}
          />
          {initialData?.imageUrl && (
            <Typography variant="body2" sx={{ mb: 1 }}>
              Current: <a href={initialData.imageUrl} target="_blank" rel="noopener">View Image</a>
            </Typography>
          )}
          <TextField type="file" fullWidth accept=".pdf" onChange={e => setPdf(e.target.files[0] || null)}
            InputProps={{ startAdornment: <InputAdornment position="start"><PictureAsPdf /></InputAdornment> }}
            sx={{ mb: 2 }}
          />
          {initialData?.pdfUrl && (
            <Typography variant="body2" sx={{ mb: 2 }}>
              Current: <a href={initialData.pdfUrl} target="_blank" rel="noopener">View PDF</a>
            </Typography>
          )}
        </DialogContent>
        <DialogActions>
